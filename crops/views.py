...
def crop_detail(request, crop_id):
    try:
        ...
        messages.error(request, _('The crop you are looking for does not exist.'))
...
def crop_delete(request, crop_id):
    try:
        ...
        messages.error(request, _('The crop you are trying to delete does not exist or you do not have permission to delete it.'))
        return redirect('dashboard')
...
def calculate_recommendations(form_data):
    # Get all crops
    ...
    climate_zone = form_data['climate_zone']
    field_area = form_data['field_area']
    soil_ph = form_data['soil_ph']
    rainfall = form_data['rainfall']
    irrigation_method = form_data['irrigation_method']
    growing_season = form_data['growing_season']
    market_preference = form_data['market_preference']
...
def crop_recommendation(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                recommended_crops = calculate_recommendations(form.cleaned_data)
