...
def respond_demand(request, demand_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'accept':
            ...
            Notification.objects.create(
                user=demand.buyer,
                title=_('Demand Accepted'),
                message=_('%(farmer)s has accepted your demand for %(crop)s') % {
                    'farmer': request.user.get_full_name(),
                    'crop': demand.crop_name
                },
                notification_type='demand_accepted',
                related_url=f'/bidding/demands/{demand.id}/'
            )
            
            # Mark demand as accepted
            demand.is_active = False
            demand.accepted_by = request.user
            demand.save()
            
            messages.success(request, _('You have accepted this demand. The buyer has been notified.'))
            return redirect('bidding:demand_list')
        elif action == 'reject':
            # Create notification for the buyer
            Notification.objects.create(
                user=demand.buyer,
                title=_('Demand Rejected'),
                message=_('%(farmer)s has rejected your demand for %(crop)s') % {
                    'farmer': request.user.get_full_name(),
                    'crop': demand.crop_name
                },
                notification_type='demand_rejected',
                related_url=f'/bidding/demands/{demand.id}/'
            )
            
            messages.success(request, _('You have rejected this demand. The buyer has been notified.'))
            return redirect('bidding:demand_list')
